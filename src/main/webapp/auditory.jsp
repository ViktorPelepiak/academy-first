...
<html>
<head>
    <title>Розклад для аудиторій</title>
    ...
    <div class="collapse navbar-collapse" id="navbarText">
        <ul class="navbar-nav mr-auto">
            <li class="nav-item active">
                <form method="post" id="test" action="/ViPSchedule/for_auditory">
                    <select class="form-control" style="width: 100px" size="1" name="auditoryId"
                            onchange="document.getElementById('test').submit()">
                        <c:forEach var="a" items="${auditories}">
                            <option value="${a.id}" <c:if test="${a.id==auditoryId}">selected</c:if>>${a.buildingNumber}-${a.auditoryNumber}</option>
                        </c:forEach>
                    </select></form>
            </li>
        </ul>
...
<c:if test="${error!=null}">
    ...
    <div class="container">
        <ul id="myTab" class="nav nav-pills nav-justified" role="tablist">
            <li class="nav-item active" data-toggle="pill" role="tab"><a class="nav-link" href="#today" role="tab" data-toggle="tab">TODAY</a></li>
            <li class="nav-item" data-toggle="pill" role="tab"><a class="nav-link" href="#tomorrow" role="tab" data-toggle="tab">TOMORROW</a></li>
            <li class="nav-item" data-toggle="pill" role="tab"><a class="nav-link" href="#first" role="tab" data-toggle="tab">UNPAIR WEEK</a></li>
            <li class="nav-item" data-toggle="pill" role="tab"><a class="nav-link" href="#second" role="tab" data-toggle="tab">PAIR WEEK</a></li>
        </ul>

        <div class="tab-content">
            <div role="tabpanel" class="tab-pane active" id="today">
                <table>
                    <tr><td colspan="2">${todayDay}</td></tr>
                        ...
                        <tr><td>${i}.</td><td>
                            ...
                            <c:forEach var="l" items="${today}">
                                <c:if test="${i == l.lessonTime}">
                                    <table>
                                        <tr><td>${l.subject}</td></tr>
                                        <tr><td>${l.groupNumber}</td></tr>
                                        <tr><td>${l.teacher}</td></tr>
                                    </table>
            ...
            <div role="tabpanel" class="tab-pane" id="tomorrow">
                <table>
                    <tr><td colspan="2">${tomorrowDay}</td></tr>
                    <c:forEach var="i" items="1,2,3,4,5">
                        <tr><td>${i}.</td><td>
                            <c:set var="flag" value="false"/>
